...
/** Returns true if the group parent or one of its children is the current page.
 *  Groups with children must NOT activate from startsWith(parent) alone —
 *  that caused User Management + Locked Users to both highlight simultaneously. */
function isGroupActive(item: NavItem, pathname: string): boolean {
    if (item.children?.length) {
        // Parent exact match OR any child match
        return (
            pathname === item.path ||
            item.children.some((c) => pathname === c.path || pathname.startsWith(c.path + '/'))
        );
    }
    // Leaf: root special-case, otherwise exact or nested
...
// ─── NavRow: a top-level nav button (with optional expand toggle) ──────────────
...
const NavRow = ({ item, isActive, isOpen, onToggle, onNavigate }: NavRowProps) => {
    const hasChildren = !!item.children?.length;

    const handleClick = () => {
        if (hasChildren) {
            onToggle();   // expand / collapse
            onNavigate(); // also navigate to parent page
        } else {
            onNavigate();
        }
    };
            ...
            {item.badge && (
                <span
                    className={`${styles.navBadge} ${
                        item.badgeVariant === 'teal' ? styles.navBadgeTeal : ''
                    }`}
                >
                    {item.badge}
                </span>
...
const SubNavRow = ({ item, isActive, onNavigate }: SubNavRowProps) => (
    <button
        onClick={onNavigate}
...
export const Sidebar = () => {
    ...
    // Auto-open any group whose path (or child path) is currently active.
    const [openGroups, setOpenGroups] = useState<Record<string, boolean>>(() => {
        const initial: Record<string, boolean> = {};
        navItems.forEach((section) =>
            section.items.forEach((item) => {
                if (item.children?.length && isGroupActive(item, location.pathname)) {
                        ...
                        {section.items.map((item) => {
                            ...
                            return (
                                <div key={item.label}>
                                    <NavRow
                                        item={item}
                                        isActive={hasChildren ? groupActive : isSelfActive}
                                        isOpen={isOpen}
                                        onToggle={() => toggleGroup(item.path)}
                                        onNavigate={() => navigate(item.path)}
                                    />

                                    {/* Sub-items with smooth slide */}
                                    {hasChildren && (
                                        <div
                                        ...
                                        >
                                            {item.children!.map((child) => (
                                                <SubNavRow
                                                    ...
                                                    onNavigate={() => navigate(child.path)}
                                                />
                                            ))}
                                        </div>
                                    )}
